...
/**
 * Loads bot config from env and validates it up front.
 *
 * Authorization model: this bot is designed to run as a linked device on
 * YOUR OWN WhatsApp account. Commands are only accepted from your own
 * "Message Yourself" self-chat (fromMe === true AND remoteJid === your own
 * JID) — see index.ts. That's what actually gates who can issue commands:
 * only someone with access to your phone/linked session can produce a
 * fromMe message in the first place. There is deliberately no numeric
 * allowlist here, since the self-chat check already scopes this to you.
 */
export function loadConfig(): BotConfig {
  const defaultComputerId = process.env.DEFAULT_COMPUTER_ID;
  if (!defaultComputerId) {
